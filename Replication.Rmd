...
#teams don't play the same amount of games, there are 26 games in the first round then they are divided in top teams and bottom teams. isolate the 26 first games (Appendix A.2, in the paper) the first season
...
team_rank_r27 <- data %>%
  filter(played == 27) %>%           # only round 26
  distinct(team, team_rank)          # one row per team with its rank
...
```
```{r}
# Base priors for all fixed-effects specifications
priors_fixef_base <- c(
...
priors_fixef_col7 <- c(
  ...
  set_prior("normal(0, 0.8)", class = "b"),           # all dummy/FE coefficients
